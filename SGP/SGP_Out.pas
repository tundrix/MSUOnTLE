...
Procedure Output_Angle(angle : double;
  ...
  if D_M_S then
    ...
    if degrees then
      case dec of
 //         0 : Write(fout,deg:width,'є');
 //       1,2 : Write(fout,deg:width,'є',TwoDigit(min),'''');
 //       3,4 : Write(fout,deg:width,'є',TwoDigit(min),'''',TwoDigit(sec),'"');
          0 : Write_box('deg:width,',floattostr(deg),'є');
        1,2 : Write_box('deg:width,',floattostr(deg),'є',TwoDigit(min),'''');
        3,4 : Write_box('deg:width,',floattostr(deg),'є',TwoDigit(min),'''',TwoDigit(sec),'"');
        end {case}
    else
      case dec of
