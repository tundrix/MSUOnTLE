...
Function Select_Option(menu : options; number,x,y,w,h : byte) : byte;
  ...
  procedure intr(i:integer;r:registers);

  function wherex():integer;
  function wherey():integer;
  procedure GotoXY(i:integer;r:integer);

  procedure TextColor(i:integer);
  procedure TextBackground(i:integer);
  procedure Window(a1:integer;a2:integer;a3:integer;a4:integer);
  procedure ClrScr();
  ...
  procedure ClrEOL();
      ...
      procedure NoSound;
         ...
         procedure GetDate(var a1,a2,a3,a4:word);
         procedure GetTime(var a1,a2,a3,a4:word);

function        ReadKey():char;


          procedure FindFirst    (pattern:string;    AnyFile :integer    ;dirinfo:searchrec);
                    procedure FindNext(dirinfo:searchrec);
function  AnyFile():integer;
function          DosError():integer;


//-------------------

procedure Write(w:word);    overload;
procedure Write(s:string);    overload;
procedure Write(s1,s2,s3:string);    overload;
//procedure Write(s1:text;s2,s3:string);    overload;

procedure Write_box(s1,s2,s3,s4,s5,s6,s7:string);    overload;

procedure Write_box(s1,s2,s3,s4,s5,s6:string);    overload;

procedure Write_box(s1,s2,s3,s4,s5:string);    overload;
procedure Write_box(s1,s2,s3,s4:string);    overload;
procedure Write_box(s1,s2,s3:string);    overload;
procedure Write_box(s1,s2:string);    overload;
procedure Write_box(s:string);    overload;
...
IMPLEMENTATION
  Uses {CRT,DOS,}MinMax, windows, sysutils;
...
var
  ...
  begin
         ...
         procedure GetDate(var a1,a2,a3,a4:word);
         begin
           DecodeDateFully(now(),a1,a2,a3,a4);
         end;

         procedure GetTime(var a1,a2,a3,a4:word);
         begin
           DecodeTime(now(),a1,a2,a3,a4);
           a4:=a4 div 10;
         end;
...
procedure Write_box(s1,s2,s3,s4,s5,s6,s7:string);
begin
  messagebox(0,pchar(s1),pchar(s2+s3+s4+s5+s6+s7),0);

end;
procedure Write_box(s1,s2,s3,s4,s5,s6:string);
begin
  messagebox(0,pchar(s1),pchar(s2+s3+s4+s5+s6),0);

end;
procedure Write_box(s1,s2,s3,s4,s5:string);
begin
  messagebox(0,pchar(s1),pchar(s2+s3+s4+s5),0);

end;
procedure Write_box(s1,s2,s3,s4:string);
begin
  messagebox(0,pchar(s1),pchar(s2+s3+s4),0);

end;
procedure Write_box(s1,s2,s3:string);
begin
  messagebox(0,pchar(s1),pchar(s2+s3),0);

end;
procedure Write_box(s1,s2:string);
begin
  messagebox(0,pchar(s1),pchar(s2),0);

end;

procedure Write_box(s:string);
begin
